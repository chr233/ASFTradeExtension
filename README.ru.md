...
3. Перезапустить `ArchiSteamFarm` , и используйте команду  `CTE`  для проверки работоспособности плагина
...
- `CTEVERSION` / `CTEV` проверить последнюю версию ASFTradeExtension
- `CTEUPDATE` / `CTEU` автоматическое обновление ASFTradeExtension (возможно, потребуется обновить ASF вручную)
...
> Если некоторые команды отключены, то все равно можно вызвать команду в виде `CTE.xxx`, например `CTE.FULLSETLIST`
...
| ------------------- | ---------- | --------------- | ------------------------------------------------------------------------------- |
| `ASFTradeExtension` | `CTE`      | `FamilySharing` | Получить версию ASFTradeExtension                                               |
| `CTEVERSION`        | `CTEV`     | `Operator`      | Проверить последнюю версию ASFTradeExtension                                    |
| `CTEUPDATE`         | `CTEU`     | `Owner`         | Обновить ASFTradeExtensionдо последней версии (необходим ручной перезапуск ASF) |
...
| `FULLSET [Bots] <appIds>`                      | `FS`       | `Operator` | Отображение информации о количестве карточек в данном приложении                                                        |
| `SENDCARDSET [Bots] AppId SetCount TradeLink`  | `SCS`      | `Master`   | Отправляет карточку с указанным `SetCount` набором с указанным `AppId` на торговую ссылку.                              |
| `2SENDCARDSET [Bots] AppId SetCount TradeLink` | `2SCS`     | `Master`   | Аналогично `SENDCARDSET`, автоматически подтверждает транзакцию после ее отправки (требуется настройка 2FA)             |
...
| `CSSENDITEM [Bots]`                      | `CSI`      | `Master`   | Отправить инвентарь CSGO ботов другому боту                                                                             |
| `2CSSENDITEM [Bots]`                     | `2CSI`     | `Master`   | Аналогично `CSSENDITEM`, автоматически подтверждает транзакцию после ее отправки (требует настройки 2FA)                |
| `CSSENDITEM [Bots] ClassId CountPerBot`  | `CSI`      | `Master`   | Отправить инвентарь CSGO ботов другому боту, укажите `ClassId` предмета и количество предметов, полученных каждым ботом   |
| `2CSSENDITEM [Bots] ClassId CountPerBot` | `2CSI`     | `Master`   | Аналогично `SENDCARDSET`, автоматически подтверждает транзакцию после ее отправки (требуется настройка 2FA)             |
