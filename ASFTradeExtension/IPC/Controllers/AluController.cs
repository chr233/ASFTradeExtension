...
using ASFLevelUpBot.IPC.Response;
...
public sealed class AluController : AbstractController
{
        ...
        [Description("机器人名称")] string botName)
    {
        ...
        CardSetCache.MasterBotName = botName;
        await CardSetCache.SaveFile().ConfigureAwait(false);
        ...
        [Description("强制刷新")] bool forceReload = false)
    {
        ...
        var inv = await handler.InvHandler.GetCardSetCache(forceReload).ConfigureAwait(false);
        await handler.InvHandler.FullLoadAppCardGroup(inv).ConfigureAwait(false);
        ...
        [Description("自动确认")] bool autoConfirm = true)
    {
        ...
        var (bot, handler) = Utils.GetRandomBot();
        if (bot == null || handler == null)
            return Ok(new GenericResponse(false, "未设置发货机器人"));
        ...
        try
        {
            ...
            if (targetLevel <= badgeInfo.Level)
                return Ok(new GenericResponse(false, $"用户 {badgeInfo.Nickname} {targetSteamId} 的等级 {badgeInfo.Level} 大于等于目标等级"));

            var needExp = CalcExpToLevel(badgeInfo.Level, targetLevel, badgeInfo.Experience);
            var needSet = needExp / 100 + 1;

            var avilableSets = await handler.InvHandler.SelectFullSetCards(badgeInfo.Badges, needSet).ConfigureAwait(false);
            ...
            await handler.InvHandler.AddInTradeItems(avilableSets.TradeItems).ConfigureAwait(false);
            ...
            var response = new SendCardSetResponse(badgeInfo.Nickname, badgeInfo.Level, badgeInfo.Experience, targetLevel, avilableSets.CardSet, avilableSets.CardCount, avilableSets.CardType, autoConfirm);

            return Ok(new GenericResponse<SendCardSetResponse>(true,
                $"发送报价成功, 从 {badgeInfo.Level} 级升级到 {targetLevel} 级还需要 {needExp} 点经验, 需要合成 {needSet} 套卡牌, 总计发送了 {response.CardSet} 套 {response.CardCount} 张卡牌", response));
        }
        catch (Exception ex)
        {
